...
if [ -f "$WINEPREFIX/drive_c/Program Files/MetaTrader 5/terminal64.exe" ]; then
    ...
    # [mt5-terminal] refresh the bundled EA in case the image shipped a newer build
    cp -f /SocketBridgeEA.ex5 "$WINEPREFIX/drive_c/Program Files/MetaTrader 5/MQL5/Experts/SocketBridgeEA.ex5" 2>/dev/null || true
