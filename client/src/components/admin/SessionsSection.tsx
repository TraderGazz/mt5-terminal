  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {sessions && sessions.length === 0 && (
        <AdminCard className="p-6 text-center text-[14px] text-text-secondary">Нет активных сессий</AdminCard>
      )}
      ...
      {sessions && sessions.length > 0 && (
        <>
          ...
          <div className="flex flex-col gap-3 md:hidden">
            {sessions.map((s) => (
              <div key={s.id} className="rounded-[10px] bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.04)]">
                <div className="flex items-center gap-2">
                  <span className="text-[15px] font-medium text-black">{s.name || s.login}</span>
                  <Pill tone={ROLE_TONE[s.role] ?? 'gray'}>{s.role}</Pill>
                  {s.user_id === selfId && (
                    <span className="text-[11px] text-text-secondary">(вы)</span>
                  )}
                </div>
                <p className="tnum text-[12px] text-text-secondary">логин {s.login}</p>
                <div className="mt-3 grid grid-cols-2 gap-x-3 gap-y-2 text-[13px]">
                  <div>
                    <p className="text-text-secondary">Устройство</p>
                    <p className="text-black">{deviceLabel(s.user_agent)}</p>
                  </div>
                  <div>
                    <p className="text-text-secondary">IP</p>
                    <p className="tnum text-black">{s.ip || '—'}</p>
                  </div>
                  <div className="col-span-2">
                    <p className="text-text-secondary">Последняя активность</p>
                    <p className="tnum text-black">{formatDateTime(new Date(s.last_seen_at).getTime())}</p>
                  </div>
                </div>
                <div className="mt-3 border-t border-separator pt-3">
                  <AdminButton variant="destructive" className="w-full" onClick={() => setKickTarget(s)}>
                    Кикнуть
                  </AdminButton>
                </div>
              </div>
