...
// Каждый опрос перекачивает ВЕСЬ /history/raw (тысячи строк, ~230КБ сжатым)
// — на активном счёте 20с ощутимо нагружали сеть при долго открытой
// странице (наблюдали 6.5МБ за одну сессию в DevTools). WS 'trade' событие
// уже даёт быстрый отклик на реальную новую сделку (когда EA его шлёт);
// этот поллинг — только страховка на случай, если оно не пришло.
